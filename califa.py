...
def showspectra(event):
    ...
    try:
        ...
        if token==0:
            ...
            ax2.plot(wavelength, [data[cout][x][y] for cout in range(begin_wl,end_wl,step)])
...
def compare(wavelength,ran,step):
    ...
    wavelength=[wavelength[x] for x in range(begin_wl,end_wl,step)]
...
while True:
    ...
    if ran != None:
        ...
        for x in range(len(data[0])):
            for y in range(len(data[0][0])):
                for z in range(0,len(data),step):
                    if begin_wl<=z<=end_wl:
                        pass
                    else:
                        data[z][x][y]=0
    else:
        wavelength = [wavelength[x] for x in range(0,len(data),step)]
        pass
    ...
    for x in range(len(data[0])):
        for y in range(len(data[0][0])):
            magnitude_black.append(sum([data[cout][x][y] for cout in range(0,len(data),step)]))
    ...
    ax1.set_yticks(range(0,len(data[0][0])-10, 6))
    ax1.set_yticklabels([declination[x] for x in range(0,len(declination),6)])
    ...
    ax1.set_xticks(range(0, len(data[0])-1, 4))
    ax1.set_xticklabels([RA[x] for x in range(0,len(RA),4)], rotation=90)
